\AuthorFirstName{Graduate}
\AuthorLastName{Student}

% Title can be up to three lines of no more than 48 characters per line
\ThesisTitle{Title first part}{continued title}{last part of title}
\GraduateDepartment{Physics}

\BachelorDegreeType{B.S.}
\BachelorDegreeMajor{Physics}
\BachelorDegreeUniversity{Undergraduate University}

\MasterDegreeType{M.S.}
\MasterDegreeMajor{Physics}
\MasterDegreeUniversity{Southern Methodist University}

\ThesisDefenseDateYear{2019}
\ThesisDefenseDateMonth{January}
\ThesisDefenseDateDay{1}

\GraduationDateYear{2019}
\GraduationDateMonth{May}
\GraduationDateDay{1}

\SMUCollege{Dedman College}

% Committee
\AdvisorFullName{Dr.~Advisor Name}
\AdvisorTitle{Associate Professor}

\CommitteeMemberA{Dr.~Committee Chair}
\CommitteeMemberTitleA{Professor}

\CommitteeMemberB{Dr.~Committee Member}
\CommitteeMemberTitleB{Associate Professor}

\CommitteeMemberC{Dr.~External Member}
\CommitteeMemberTitleC{Assistant Professor}
